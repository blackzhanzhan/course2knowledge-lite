...
def _resolve_course_id(arguments: dict[str, Any], store: SQLiteCourseStore) -> str:
    ...
    candidates = [
        {
            ...
            "title": str(course.get("title") or course.get("course_id") or "").strip(),
        }
...
def _import_status_get_handler(arguments: dict[str, Any], **_registry_kwargs: Any) -> str:
    try:
        ...
        payload = SQLiteCourseStore(_store_root(arguments)).read_import_status(import_id)
...
def _lecture_id_from_arguments(store: SQLiteCourseStore, course_id: str, arguments: dict[str, Any]) -> str:
    lecture_id = str(arguments.get("lecture_id", "") or "").strip()
    if lecture_id:
        return lecture_id
    lecture_sequence = arguments.get("lecture_sequence")
